...
public class MultimodalController {
    ...
    @Autowired // Added ObjectMapper injection
    ...
    * <b>File requirements:</b>
    ...
    public ResponseEntity<?> chatMultimodal(
        ...
        try {
            ...
            // Convert multimodal content to JSON string before setting
            String multimodalContentString = objectMapper.writeValueAsString(multimodalContent);
            userMsg.setContent(multimodalContentString);
            
            // Check for recent similar chats to avoid duplicates
            List<Chat> recentChats = chatService.getRecentChats(5);
            Chat existingChat = null;
            
            for (Chat recentChat : recentChats) {
                if (!recentChat.getMessages().isEmpty() && 
                    System.currentTimeMillis() - recentChat.getCreatedAt().toEpochMilli() < 10000) { // Within 10 seconds
                    
                    // Get the first message to check
                    ChatMessage firstMsg = recentChat.getMessages().get(0);
                    if ("user".equals(firstMsg.getRole()) && 
                        "multipart/mixed".equals(firstMsg.getContentType())) {
                        
                        // Found a recent multimodal chat, reuse it
                        existingChat = recentChat;
                        break;
                    }
                }
            }
            ...
            Chat chat = existingChat != null ? existingChat : chatService.createChat(userMsg);
            
            // Get AI response
            String aiResponse = openAIService.getMultimodalCompletion(multimodalContent, modelId);
            
            // Add the AI response to the chat history - addMessageToChat already has duplicate detection
